...
class MainWindow(QMainWindow):
	def __init__(self):
		super().__init__()
		self.setWindowTitle("Garbage Graph Maker 3600")
	...
	def CreateControlPanel(self):
		...
		form.addRow("Швидкість падіння (м/с):", self.input_g)
		...
		infoGroupLayout = QHBoxLayout()
		infoGroup = QGroupBox("Інформація")
		infoGroup.setFixedWidth(300)

		self.resultLabel = QLabel("Час польоту:\nДальність:")
		self.resultDataLabel = QLabel("0.00 с.\n0.00 м.")
		self.resultDataLabel.setAlignment(Qt.AlignmentFlag.AlignRight)

		infoGroupLayout.addWidget(self.resultLabel)
		infoGroupLayout.addWidget(self.resultDataLabel)
		...
		buttonGroup = QGroupBox("Кнопки управління")
		buttonGroup.setFixedWidth(300)
	...
	def BuildGraph(self):
		...
		if y0 < 0:
			QMessageBox.warning(
				self,
				"Whooops!",
				"Здається, ми знаходимося під текстурами!\n" +
				"(Координата Y не може бути від'ємною)"
			)
			return
			...
			QMessageBox.warning(
				self,
				"Whooops!",
				"Стріляючи з пістолета - пуля виходить лише вперед, вона не залишається в дулі і не виходить назад\n" +
				"(Швидкість не може бути від'ємною або дорівнювати 0 при y = 0)"
			)
			return

		elif (angleDeg == 0 or angleDeg == 180) and y0 == 0:
			QMessageBox.warning(
				self,
				"Whooops!",
				"Фактично, зараз ми просто повземо по землі\n" +
				"(Стріляти горизонтально має сенс лише з якоїсь висоти Y0 > 0)"
			)
			return

		elif angleDeg < 0 or angleDeg > 180:
			QMessageBox.warning(
				self,
				"Whooops!",
				"Чисто теоретично, об'єкт може впасти вверх... На іншій стороні землі\n" +
				"(Кут не може бути меншим за 0, або більшим за 180)"
			)
			return
			...
			QMessageBox.warning(
				self,
				"Whooops!",
				"А зараз ми з вами знаходимося в космосі...\n" +
				"(Швідкість вільного падіння повинна бути більше 0)"
			)
			return

		mathModule = MathModule(x0, y0, v0, angleDeg, fps, g)
		x, y, totalTime, distance = mathModule.DoCalculations()

		self.resultDataLabel.setText(f"{totalTime:.2f} с.\n{distance:.2f} м.")
	...
	def TimerTick(self):
		...
		if progress >= 1.0:
			...
			self.timer.stop()
			self.currentLine.set_data(self.currentX, self.currentY)
			self.axes.legend(loc='upper left')
			self.canvas.draw() 
			return
		else:
